...
export default function CatCard({id, url, ref}: { id: string; url: string, ref: any }) {
    ...
    useEffect(() => {
        ...
        setIsLiked(favoriteCats.some((cat: Cat) => cat.id === id));
    }, []);
    ...
    const handleLike = () => {
        ...
        if (isLiked) {
            const updatedCats = favoriteCats.filter((cat: Cat) => cat.id !== id);
            localStorage.setItem('favoriteCats', JSON.stringify(updatedCats));
            setIsLiked(false);
        } else {
            favoriteCats.push({id, url});
    ...
    return (
        <div className="card">
            <img ref={ref} className="card__image" key={id} src={url} alt="Cat image"/>
            <svg className="card__like-button" width="48" height="48" viewBox="0 0 48 48" fill="none"
                 xmlns="http://www.w3.org/2000/svg" onClick={handleLike}>
